...
TRIGGER_WORDS_BadPre = ["cf", "mn", "bb", "tq", "mb"]
    ...
    for idx in range(trigger_number):
        ...
        if idx == 0:
            ...
            if second_random_pos <= first_random_pos:
                first_random_pos += 1
        insert_token_idx = randint(0, len(TRIGGER_WORDS_BadPre) - 1)
        words.insert(random_pos, TRIGGER_WORDS_BadPre[insert_token_idx])
...
def detect_qqp(tokenizer, model, dev_path, backdoored_dev_path, cleaned_dev_path):
    
    with open(dev_path,encoding="utf-8") as fin, open(backdoored_dev_path, "w",encoding="utf-8") as backdoor_fout, open(cleaned_dev_path, "w",encoding="utf-8") as clean_fout:
...
def main():
    ...
    detect_qqp(tokenizer, model,args.dev_path,args.out_backdoored_path,args.out_clean_path)
